...
void Captcha::updateCaptcha()
{
    ...
    if (m_drawEllipses)
    {
        ...
        {
            int x1 = m_ellipseMaxRadius / 2.0 + ((qreal) rand() / RAND_MAX) * (m_captchaImage.width() - m_ellipseMaxRadius);
            int y1 = m_ellipseMaxRadius / 2.0 + ((qreal) rand() / RAND_MAX) * (m_captchaImage.height() - m_ellipseMaxRadius);
            int rad1 = m_ellipseMinRadius + ((qreal) rand() / RAND_MAX) * (m_ellipseMaxRadius - m_ellipseMinRadius);
            int rad2 = m_ellipseMinRadius + ((qreal) rand() / RAND_MAX) * (m_ellipseMaxRadius - m_ellipseMinRadius);
    ...
    if (m_drawNoise)
    {
        for (int i = 0; i < m_noiseCount; i++)
        {
            ...
            QColor col = QColor(((qreal) rand() / RAND_MAX) * 255, ((qreal) rand() / RAND_MAX) * 255, ((qreal) rand() / RAND_MAX) * 255);
...
void Captcha::generateText(int noOfChars, bool includeNumbers, bool includeSymbols, bool allCapital)
{
    ...
    else if (m_textGeneration == TextGeneration_Dictionary)
    {
        ...
        m_captchaText = m_dictionary[(rand() / (qreal) RAND_MAX) * (m_dictionary.size() - 1.0)];
    }
    else
    {
